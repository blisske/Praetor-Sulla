...
- React + Vite + Tailwind frontend served by nginx at `:8085` (LAN debug)
  and `sulla.blisske.hopto.org` (public via swarm Traefik + Let's Encrypt)
- SQLite (WAL mode), bind-mounted from `~/swarm/sulla/data/`
...
```bash
# Build + bring up (run from ~/swarm/, NEVER from ~/swarm/sulla/repo/)
cd ~/swarm && docker compose up -d --build sulla-engine sulla-api sulla-web

# Logs
docker compose logs sulla-engine --tail 50
...
Sulla shares the Praetor stack with Anton and Tiberius — same 4-paradigm signal
...
| Concern | Tiberius (crypto) | Anton (equities) | Sulla (FX) |
|---|---|---|---|
